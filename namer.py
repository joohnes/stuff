...
def main():
    ...
    howMany = int(
        input("How many photos do you want to assign to one title? "))
    ...
    try:
        ...
        for x in processed:
            ...
            for i in range(howMany):
                copyfile(
                    PATH + "/" + files[counter], f"{PATH}/copied/{name}_{next(spec)}.{extension}")
                counter += 1
    except FileExistsError:
        cprint("Could not create new files!", "red")
